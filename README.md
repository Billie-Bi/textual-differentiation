...
This repository contains the code and textual inputs required to reproduce the study **“Computational analysis of textual differentiation across the tripartite structure of Virginia Woolf’s *To the Lighthouse*.”**

The repository includes the six analysis scripts used in the article, the exact cleaned text files supplied to the pipeline, and the final Python environment specification. Because the manuscript has not yet been published, generated analytical outputs are not currently included in the public repository. All output directories and files can be reproduced locally by running the pipeline. The data necessary to support the reported findings will be released no later than publication, subject to journal requirements.
...
`Textual_Indicators_output/` is created locally when the scripts are run. It is excluded from version control while the associated manuscript remains unpublished. The generated directory contains intermediate data, statistical results, visualizations, figure source data, and publication-stage tables.
...
The scripts create `Textual_Indicators_output/` automatically and write all generated files under that directory. Rerunning a script regenerates or overwrites its corresponding local outputs. The output directory is ignored by Git and is not part of the public repository.
...
The public repository provides the exact cleaned textual inputs, the final environment specification, and all six scripts required to reproduce the analysis. Running the complete pipeline locally generates:
...
Generated outputs are temporarily excluded from the public repository because the associated manuscript has not yet been published. Their absence does not change the analytical workflow or the filenames produced by the scripts. The data necessary to support the reported findings will be deposited or supplied with the article no later than publication, subject to journal requirements.
...
If you use this code, please cite the associated article and the archived repository release when they become available. Initial citation metadata are provided in `CITATION.cff`. The repository is available at [https://github.com/Billie-Bi/textual-differentiation](https://github.com/Billie-Bi/textual-differentiation). Add the Zenodo DOI and article DOI when they become available.

## License

The analysis code is licensed under the MIT License. Any author-generated numerical outputs released separately are licensed under CC BY 4.0 as described in `DATA_LICENSE.md`. Those licenses do not replace or modify the terms or public-domain status applicable to the source novels in `data/`.
