...
def run_pipeline():
    ...
    segmentation_dataframe["perspective_shift"] = (
        segmentation_dataframe["semantic_distance"] + segmentation_dataframe["pronoun_transition"]
    )
    ...
    chapter_aggregated_dataframe = (
        segmentation_dataframe.groupby(["part", "chapter_id"], observed=True)[TARGET_METRICS]
        .mean()
        .reset_index()
    )
    ...
    for plot_axis, target_metric in zip(boxplot_axes.flatten(), TARGET_METRICS):
        ...
        plot_axis.set_ylabel("Raw Values", fontsize=14)
