...
NUMBA_CACHE_DIRECTORY = os.path.abspath(
    os.environ.get("TEXTUAL_INDICATORS_NUMBA_CACHE_DIRECTORY", os.path.join(PROJECT_DIRECTORY, ".numba_cache"))
)
os.makedirs(NUMBA_CACHE_DIRECTORY, exist_ok=True)
os.environ["NUMBA_CACHE_DIR"] = NUMBA_CACHE_DIRECTORY
...
SPACY_PIPE_BATCH_SIZE = 32
EMBEDDING_BATCH_SIZE = 8
SMOOTHING_WINDOW = 3
UMAP_NEIGHBORS = 10
UMAP_MINIMUM_DISTANCE = 0.3
UMAP_COMPONENTS = 2
RANDOM_SEED = 42
FIGURE_DPI = 300
BOXPLOT_FIGURE_SIZE = (14, 10)
TRENDS_FIGURE_SIZE = (16, 12)
UMAP_FIGURE_SIZE = (10, 8)
...
def encode_long_text_for_umap(text, embedding_model):
    ...
    max_content_tokens = embedding_model.max_seq_length - tokenizer.num_special_tokens_to_add(pair=False)

    if max_content_tokens <= 0:
        raise ValueError("[ERROR] The embedding model has an invalid maximum sequence length.")
    ...
    token_chunks = [
        token_ids[start_index : start_index + max_content_tokens] for start_index in range(0, len(token_ids), max_content_tokens)
    ]
...
def extract_lexicogrammatical_features(spacy_document):
    ...
    content_words = [token.text.lower() for token in spacy_document if token.pos_ in ["NOUN", "VERB", "ADJ", "ADV"] and token.is_alpha]
...
def run_pipeline():
    ...
    for spacy_document in tqdm(
        spacy_full_model.pipe(segmentation_dataframe["text"].tolist(), batch_size=SPACY_PIPE_BATCH_SIZE),
    ...
    chapter_aggregated_dataframe[["umap_x", "umap_y"]] = umap_reducer.fit_transform(chapter_embeddings)
    ...
    for plot_axis, target_metric in zip(boxplot_axes.flatten(), TARGET_METRICS):
        ...
        plot_axis.set_ylabel("Chapter-Level Mean")
    ...
    chapter_aggregated_dataframe_sorted = chapter_aggregated_dataframe.sort_values("chapter_index").reset_index(drop=True)

    plt.figure(figsize=UMAP_FIGURE_SIZE)
    ...
    save_dataframe_to_numeric_csv(
        chapter_aggregated_dataframe_sorted[["chapter_index", "part", "chapter_id", "umap_x", "umap_y"]],
        OUTPUT_UMAP_DATA,
    )
